...
- 翻译时自动获取**近义词 / 反义词**，展示在详情页
- 翻译结果写入云端，之后打开无需重复翻译
- 可在设置中选择翻译服务：**免费翻译（MyMemory，无需 Key）** 或 **Google 翻译（自备 API Key）**
- 词库**分页加载**（每页 50 条），默认按收藏次数排序，支持游标翻页
- 搜索关键词实时过滤，并自动补全全量词库用于匹配
- 顶部操作栏提供**刷新按钮**，一键强制拉取最新数据（跳过本地缓存）
- 设置面板提供 **GitHub Issues** 反馈入口，方便提交问题或功能建议
...
8. 翻译完成后，详情页会同时展示该词的**近义词**和**反义词**（来自 Free Dictionary API，无需额外配置）
...
│   ├── icon48.png
│   ├── icon128.png
│   └── alipay-qr.jpg      # 支付宝收款码（自行放入，插件不分发）
└── README.md
```
...
**Q：近义词 / 反义词为什么有些单词没有？**  
A：近/反义词数据来自 Free Dictionary API，该 API 本身收录的词条不一定包含近/反义词字段，属于数据源限制，并非 Bug。

**Q：如何支持开发者？**  
A：点击插件右上角 **⚙️** 图标，在设置面板底部「支持开发者」区域可找到支付宝收款码。

**Q：词库数据没有及时更新怎么办？**  
A：点击顶部操作栏的 **🔄 刷新图标**，可强制跳过本地缓存、重新从 Firestore 拉取最新数据。

**Q：遇到 Bug 或有功能建议怎么反馈？**  
A：点击设置面板（**⚙️**）中的「在 GitHub 提交问题 / 功能建议」链接，或直接访问 [github.com/alanwhy/new-word/issues](https://github.com/alanwhy/new-word/issues)。

---

## 问题反馈与贡献

如果你在使用中遇到问题，或有功能建议，欢迎前往 GitHub Issues 提交：

👉 [https://github.com/alanwhy/new-word/issues](https://github.com/alanwhy/new-word/issues)

也可以直接点击插件**设置面板（⚙️）**中的「在 GitHub 提交问题 / 功能建议」入口。

---

## 支持开发者

如果这个插件对你有帮助，欢迎扫码请我喝杯咖啡 ☕

在插件设置面板（**⚙️**）底部可找到支付宝收款码。
